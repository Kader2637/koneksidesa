...
export default function Footer() {
  ...
  return (
    <footer className="bg-slate-50 pt-20 pb-12 border-t border-slate-200 text-slate-500 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-12 mb-16 relative z-10">
        ...
        <div>
          <h5 className="text-slate-800 font-bold mb-6 text-xs uppercase tracking-widest font-heading">
            Akses Langsung
          </h5>
          <ul className="space-y-4 text-sm font-semibold">
            {accessLinks.map((link) => (
              <li key={link.name}>
                <Link
                  href={link.href}
                  className="hover:text-slate-800 transition-colors duration-200 flex items-center gap-1 group w-fit"
                >
                  {link.name}
      ...
      <div className="max-w-7xl mx-auto px-6 pt-8 border-t border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4 text-xs font-bold uppercase tracking-wider text-slate-400 relative z-10">
        <p>&copy; {currentYear} Platform Ekosistem KoneksiDesa. Hak Cipta Dilindungi.</p>
        <p className="flex items-center gap-1">
          Rancang Antarmuka: 
          <span className="text-emerald-600 hover:text-emerald-700 transition-colors font-black">
