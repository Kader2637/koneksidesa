...
const DESA_RADAR_COORDS: Record<string, { x: string, y: string, shippingTo: { x: string, y: string } }> = {
  "desa agro rejo": { x: "25%", y: "42%", shippingTo: { x: "12%", y: "22%" } },
  "desa karya maju": { x: "46%", y: "62%", shippingTo: { x: "12%", y: "22%" } },
  "desa sentra kriya": { x: "78%", y: "72%", shippingTo: { x: "12%", y: "22%" } },
  "desa tani sari": { x: "56%", y: "30%", shippingTo: { x: "12%", y: "22%" } },
  "desa curug berkah": { x: "82%", y: "45%", shippingTo: { x: "12%", y: "22%" } },
  "desa cita rasa": { x: "32%", y: "75%", shippingTo: { x: "12%", y: "22%" } },
};
    ...
    const fetchHotspots = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const list = products.map((prod: any, index: number) => {
            const desaName = (prod.desa || "Desa Agro Rejo").trim();
            const key = desaName.toLowerCase();
            const coord = DESA_RADAR_COORDS[key] || {
              x: `${20 + (index * 15) % 70}%`,
              y: `${30 + (index * 12) % 60}%`,
            ...
            return {
              ...
              name: prod.name,
              desa: desaName,
              product: prod.category || "Konsumsi",
              coords: { x: coord.x, y: coord.y },
              shippingTo: coord.shippingTo,
              skor: prod.rating >= 4.8 ? "A+" : (prod.rating >= 4.5 ? "A" : "B+"),
              status: prod.stock > 0 ? "Pengiriman Aktif" : "Stok Habis"
            };
          });
          setHotspots(list);
        }
      } catch (err) {
        console.error("Error fetching map radar data:", err);
      }
    };
    fetchHotspots();
  }, []);
        ...
        <div className="absolute right-6 bottom-6 w-full max-w-sm z-30">
          <AnimatePresence mode="wait">
            {activeHotspot ? (
              ...
              >
                <MapPin className="w-7 h-7 text-slate-600 mb-3 animate-pulse text-rose-500" />
                <p className="font-bold text-slate-300">Radar Komoditas Kosong</p>
                <p className="mt-1 text-slate-500 max-w-[200px]">Belum ada aktivitas komoditas UMKM di database. Silakan jalankan seeder backend.</p>
              </motion.div>
            ) : (
              <motion.div
                key="default-card"
