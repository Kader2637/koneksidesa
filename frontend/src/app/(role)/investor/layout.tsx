...
interface InvestorContextType {
  ...
  investInCampaign: (campaignId: number, amount: number) => Promise<boolean>;
        ...
        <div className="flex items-center gap-3">
          {user?.avatar ? (
            <img
              src={user.avatar}
              alt={user.name}
              className="w-9 h-9 rounded-xl object-cover shadow-sm border border-amber-200"
            />
          ) : (
        ...
        <div className="flex-1 flex flex-col lg:ml-60 xl:ml-64 min-h-screen">
          ...
          <header className="sticky top-0 z-20 bg-white/90 backdrop-blur-md border-b border-slate-200 px-4 sm:px-6 h-14 flex items-center gap-4 shadow-sm">
            ...
            <div 
              ...
              className="hidden md:flex items-center gap-2 bg-slate-100 border border-slate-200 cursor-pointer hover:border-amber-400/50 px-3 py-1.5 rounded-lg transition-all duration-200 w-48"
            >
            ...
            <div className="flex items-center gap-2 pl-2 border-l border-slate-200">
              {user?.avatar ? (
                <img
                  src={user.avatar}
                  alt={user.name}
                  className="w-7 h-7 rounded-lg object-cover shadow-sm border border-amber-250"
                />
              ) : (
