...
export default function KatalogPage() {
  ...
  const handleInvestSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(amt) || amt <= 0) {
      toast.warning("Masukkan nominal investasi yang valid!");
      return;
    }

    if (amt > walletBalance) {
      toast.error("Saldo dompet Anda tidak mencukupi! Silakan lakukan Top Up di menu Dompet.");
      return;
    }
    ...
    if (success) {
      ...
      toast.error("Gagal melakukan investasi! Pastikan nominal tidak melebihi kapasitas penggalangan dana.");
    }
  };
