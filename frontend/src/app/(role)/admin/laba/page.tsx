...
export default function LabaPage() {
  ...
  const handleWithdraw = (e: React.FormEvent) => {
    ...
    if (amt > villageTreasury) {
      toast.error("Saldo Kas Desa tidak mencukupi!");
      return;
    }
    addWithdrawal(amt);
    setWithdrawAmt("");
    toast.success(`Sukses mengajukan penarikan sebesar Rp ${amt.toLocaleString("id-ID")} ke rekening BUMDes!`);
  };
    ...
    >
      ...
      <div className="bg-slate-950 rounded-[2rem] border border-slate-800 overflow-hidden shadow-lg">
        <div className="p-6 border-b border-slate-800/80 flex justify-between items-center bg-slate-950/80">
          ...
          <button 
            onClick={() => {
              const headers = ["ID Penarikan", "Tanggal Pencairan", "Rekening Tujuan", "Nominal Penarikan", "Status"];
              ...
              exportToPDF("Laporan Riwayat Penarikan Dana Operasional BUMDes", headers, rows, "laporan_penarikan_bumdes");
            }}
