...
const DESA_COORDINATES: Record<string, { lat: number, lng: number, color: string, sektor: string, stats: string, icon: string }> = {
  "desa agro rejo": { lat: -7.50, lng: 110.22, color: "#10b981", sektor: "tani", stats: "82 UMKM", icon: "tractor" },
  "desa karya maju": { lat: -6.89, lng: 109.13, color: "#3b82f6", sektor: "kriya", stats: "24 UMKM", icon: "truck" },
  "desa sentra kriya": { lat: -7.00, lng: 110.82, color: "#3b82f6", sektor: "kriya", stats: "15 UMKM", icon: "palette" },
  "desa tani sari": { lat: -7.80, lng: 112.01, color: "#10b981", sektor: "tani", stats: "40 UMKM", icon: "shopping-bag" },
  "desa curug berkah": { lat: -8.11, lng: 113.20, color: "#f59e0b", sektor: "wisata", stats: "10 UMKM", icon: "image" },
  "desa cita rasa": { lat: -6.50, lng: 107.50, color: "#10b981", sektor: "tani", stats: "33 UMKM", icon: "coffee" },
};

const defaultUmkms: UmkmPoint[] = [
  { 
    lat: -6.89, 
    lng: 109.13, 
...
export default function MapPage() {
  ...
  const [umkmPoints, setUmkmPoints] = useState<UmkmPoint[]>([]);
    ...
    const fetchMapData = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const grouped: Record<string, { name: string, desc: string[], sektor: string, count: number }> = {};
          
          products.forEach((prod: any) => {
            const desaName = (prod.desa || "Desa Agro Rejo").trim();
            const key = desaName.toLowerCase();
            if (!grouped[key]) {
              grouped[key] = {
                name: desaName,
                desc: [],
                sektor: prod.category === "Kerajinan" || prod.category === "Dekorasi" ? "kriya" : "tani",
                count: 0
              };
            }
            grouped[key].count += 1;
            if (grouped[key].desc.length < 2) {
              grouped[key].desc.push(prod.name);
            }
          });

          const points: UmkmPoint[] = Object.keys(grouped).map((key) => {
            const group = grouped[key];
            const coord = DESA_COORDINATES[key] || {
              lat: -7.0 + (Math.random() - 0.5) * 1.5,
              lng: 110.0 + (Math.random() - 0.5) * 1.5,
              color: group.sektor === "kriya" ? "#3b82f6" : "#10b981",
              sektor: group.sektor,
              stats: `${group.count} Produk`,
              icon: group.sektor === "kriya" ? "palette" : "tractor"
            };

            return {
              lat: coord.lat,
              lng: coord.lng,
              name: `Sentra ${group.name}`,
              desa: group.name,
              color: coord.color,
              icon: coord.icon,
              stats: `${group.count} Produk Aktif`,
              desc: `Menyediakan produk unggulan desa seperti: ${group.desc.join(", ")}.`,
              sektor: coord.sektor
            };
          });

          setUmkmPoints(points);
        } else {
          setUmkmPoints([]);
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const filteredPoints = umkmPoints.filter((spot) => {
      const matchSearch =
  ...
  }, [isMapLoaded, searchQuery, selectedSektors, umkmPoints]);
          ...
          <div className="flex flex-col md:flex-row justify-between items-start gap-4">
            ...
            >
              <div className="space-y-1">
                ...
                {umkmPoints.length === 0 && (
                  <p className="text-rose-500 dark:text-rose-400 text-[10px] font-bold bg-rose-50 dark:bg-rose-950/20 p-3 rounded-2xl border border-rose-100 dark:border-rose-900/30">
                    Belum ada data sentra komoditas di database. Silakan jalankan seeder backend.
                  </p>
                )}
              </div>
